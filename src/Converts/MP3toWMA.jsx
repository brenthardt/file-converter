...
function MP3toWMA() {
  ...
  const convertToWMA = async () => {
    ...
    try {
      setConversionStatus("Converting to WMA...");
      const ffmpeg = ffmpegRef.current;
      ...
      const fileData = await ffmpeg.readFile("output.wma");
      setConvertedFileUrl(
        URL.createObjectURL(
          new Blob([fileData.buffer], { type: "audio/x-ms-wma" })
        )
      );
